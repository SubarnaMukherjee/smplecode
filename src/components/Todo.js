...
function Todo(){  
  ...
  <>
   ...
   <div className="child-div text-center">
          <figure>
            <h5 className="subTitle"> The Task List Is Here </h5>
          </figure>
        </div>
        ...
        <button type="button" className="btn btn-secondary" onClick={() => dispatch(addTask(inputTask, inputDesc), setInputTask(''),setInputDesc(''), ("Successfully Added"))}>
        ...
        <i class=" " title="Delete Item" onClick={()=>{dispatch(deleteTask(elem.id)), alert("Successfully Deleted")}}></i>
        </div>
        )  
        })
        }
